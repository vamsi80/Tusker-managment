...
export async function recordActivity(options: RecordActivityOptions) {
  ...
  try {
    ...
    // Include newData/oldData in metadata so mobile push notifications carry entity details
    if (newData) {
      metadata = { ...(metadata || {}), newData };
    }
    if (oldData) {
      metadata = { ...(metadata || {}), oldData };
    }
    ...
    if (workspaceId) {
      let userName = providedName || "Someone";
      if (action === "MEMBER_INVITED" || action === "MEMBER_REMOVED") {
        userName = "System";
      }
