 ...
 */
export async function getAdminDailyReports(workspaceId: string, actorId: string, date?: Date, userId?: string, skip = 0, take = 50) {
    ...
    const reports = await getDb().dailyReport.findMany({
        where: {
            workspaceId,
            ...(date ? { date } : {}),
            ...(userId ? { userId } : {})
...
export async function getAdminAllReportEntries(workspaceId: string, date?: Date, userId?: string, skip = 0, take = 100) {
    ...
    const entries = await getDb().dailyReportEntry.findMany({
        where: {
            report: {
                workspaceId,
                ...(date ? { date } : {}),
