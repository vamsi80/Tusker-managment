...
myspace.get("/", async (c) => {
    ...
    try {
        ...
        const todos = await prisma.memberTodo.findMany({
...
myspace.post("/", async (c) => {
    ...
    try {
        ...
        if (typeof text === "string" && text.trim() !== "") {
            await prisma.memberTodo.create({
                data: {
                    memberId: member.id,
                    text: text.trim(),
                    completed: false,
        ...
        // Action 2: Delete a single Todo
            ...
            await prisma.memberTodo.deleteMany({
                where: {
                    id: deleteTodoId,
                    memberId: member.id
                }
        ...
        // Action 3: Toggle completed status of a single Todo
            ...
            await prisma.memberTodo.updateMany({
                where: {
                    id: todoId,
                    memberId: member.id
                },
        ...
        // Action 4: Fallback to old full list sync
            ...
            for (const todo of todos) {
                ...
                if (isTempId) {
                    ...
                    await prisma.memberTodo.upsert({
                        where: { id: todo.id },
                        update: {
                            text: todo.text,
                            completed: todo.completed,
        ...
        const updatedTodos = await prisma.memberTodo.findMany({
            where: { memberId: member.id },
            orderBy: { createdAt: "desc" }
        });
