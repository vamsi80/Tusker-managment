...
procurementIndents.get("/units", async (c) => {
  ...
  const units = await getDb().unitOfMeasure.findMany({
    where: { workspaceId },
    orderBy: [
      { isDefault: "desc" },
      { abbreviation: "asc" }
...
procurementIndents.get("/", async (c) => {
  ...
  const indents = await getDb().indent.findMany({
    where: { workspaceId },
    include: {
...
procurementIndents.get("/:id/items/:itemId/suggested-vendors", async (c) => {
  ...
  const lineItem = await getDb().indentLineItem.findFirst({
    where: {
      id: itemId,
      indentId: indentId,
      indent: { workspaceId }
