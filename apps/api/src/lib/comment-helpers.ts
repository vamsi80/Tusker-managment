...
export async function getCommentDepth(commentId: string): Promise<number> {
    ...
    while (currentId) {
        const comment: { parentCommentId: string | null } | null = await getDb().comment.findUnique({
            where: { id: currentId },
...
export async function getTaskComments(taskId: string, includeDeleted = false) {
    return await getDb().comment.findMany({
        where: {
            taskId,
...
export async function createComment(data: {
    ...
    return await getDb().comment.create({
        data: {
...
export async function editComment(
    ...
    return await getDb().comment.update({
        where: { id: commentId },
        data: {
            content: newContent,
            isEdited: true,
...
export async function getTaskCommentCount(taskId: string): Promise<number> {
    return await getDb().comment.count({
        where: {
            taskId,
            isDeleted: false,
        }
