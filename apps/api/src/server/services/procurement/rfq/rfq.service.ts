...
export class RFQService {
  static async sendRFQ(
    ...
    // Transition state
    await getDb().indentLineItem.update({
      where: { id: lineItemId },
      data: {
        status: "RFQ_SENT",
        rfqSentAt: new Date(),
  ...
  static async submitQuote(
    ...
    // Create quote
    const quote = await getDb().vendorQuote.upsert({
      where: {
        lineItemId_vendorId: {
  ...
  static async approveQuote(quoteId: string, userId: string, workspaceId: string) {
    const quote = await getDb().vendorQuote.findUnique({
      where: { id: quoteId },
  ...
  static async rejectQuote(quoteId: string, reason: string, userId: string, workspaceId: string) {
    ...
    return getDb().vendorQuote.update({
      where: { id: quoteId },
      data: {
        status: "REJECTED",
