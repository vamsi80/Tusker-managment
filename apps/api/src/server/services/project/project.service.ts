...
import { projectSchema, editProjectSchema, ProjectSchemaType, EditProjectSchemaType } from "@tusker/shared";
...
export class ProjectService {
  ...
  static async updateMemberRole(userId: string, projectId: string, targetUserId: string, newRole: PrismaProjectRole) {
    ...
    if (newRole === "PROJECT_MANAGER" && targetMember.projectRole !== "PROJECT_MANAGER") {
      await getDb().projectMember.updateMany({
  ...
  static async getProjectDashboardData(workspaceId: string, slug: string, userId: string) {
    ...
    const permissions = await getUserPermissions(workspaceId, project.id, userId);
