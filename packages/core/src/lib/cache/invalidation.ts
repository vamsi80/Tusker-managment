...
export async function invalidateUserWorkspaces(userId: string) {
    ...
    await Promise.all(tags.map((tag: string) => revalidateTag(tag, "layout" as any)));
    await revalidatePath("/w", "layout");
}
...
export async function invalidateWorkspaceTags(workspaceId: string) {
    ...
    await Promise.all(tags.map((tag: string) => revalidateTag(tag, "layout" as any)));
    await revalidatePath(`/w/${workspaceId}`, "layout");
}
...
export async function invalidateAllProjectTasks() {
    await revalidateTag('project-tasks-all', "layout" as any);
}
...
export async function invalidateAllWorkspaceTasks() {
    await revalidateTag('workspace-tasks-all', "layout" as any);
}
...
export async function invalidateAllTaskSubTasks() {
    await revalidateTag('task-subtasks-all', "layout" as any);
}
...
export async function invalidateAllComments() {
    await revalidateTag('comments-all', "layout" as any);
}
...
export async function invalidateAllActivities() {
    await revalidateTag('activities-all', "layout" as any);
}
