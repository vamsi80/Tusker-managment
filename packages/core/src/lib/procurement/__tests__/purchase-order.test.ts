...
describe("buying entity readiness", () => {
  ...
  // The logos are referenced as absolute URLs, so they must exist in the web
  // app's public/ dir even though the constants that name them live here.
  ...
  test("every entity's logo file actually exists in /public", () => {
    for (const entity of Object.values(BUYER_COMPANIES)) {
      const file = path.join(WEB_PUBLIC, entity.logo);
