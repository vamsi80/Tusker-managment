...
/**
 * Loads a task with its access check. Exported as getTaskById for the single-task
 * endpoint, which needs the task and permissions but none of the detail pages.
 */
...
export async function getTaskDetail(
    ...
    const [subtaskResults, comments, activities] = await Promise.all([
        TasksService.getSubTasksByParentIds(
            [taskId],
