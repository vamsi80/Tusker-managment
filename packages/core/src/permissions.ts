/**
 * Pure permission resolution: no session lookup, no Next.js coupling.
 * Callers supply the userId — apps/web resolves it from the session,
 * the Hono API from its authenticated context.
 */
...
export type WorkspacePermissionsType = Awaited<ReturnType<typeof fetchWorkspacePermissions>>;
export type UserPermissionsType = Awaited<ReturnType<typeof fetchUserPermissions>>;
