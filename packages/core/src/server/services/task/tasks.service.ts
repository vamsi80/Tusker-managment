...
import { parseIST } from "../../../lib/date-utils";
...
export class TasksService {
  ...
  static async bulkUploadTasksAndSubtasks({
    ...
    const { fetchUserPermissions } = await import("../../../permissions");
    const permissions = await fetchUserPermissions(project.workspaceId, projectId, userId);
  ...
  public static async resolveTaskPermissions(
    ...
    // Permission resolution is explicit about who is asking: this service no
    // longer falls back to reading the session, so a missing user is a bug in
    // the caller rather than a reason to resolve permissions for nobody.
    if (!userId) {
      throw AppError.Forbidden("A user is required to resolve task permissions.");
    }
    ...
    if (projectId) {
      ...
      const wsPerms = await fetchWorkspacePermissions(workspaceId, userId);
  ...
  static async updateTaskAssignee({
    ...
    const [task, permissions] = await Promise.all([
      ...
      fetchUserPermissions(workspaceId, projectId, userId),
    ]);
  ...
  static async getTaskById(taskId: string, workspaceId: string, projectId: string, userId: string) {
    const { fetchUserPermissions } = await import("../../../permissions");
    const permissions = await fetchUserPermissions(workspaceId, projectId, userId);
