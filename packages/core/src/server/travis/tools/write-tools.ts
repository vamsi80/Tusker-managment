...
import { nextMaterialId } from "../../services/procurement/procurement-entity-id";
import { SchemaType, type FunctionDeclaration } from "@google/generative-ai";
import { z } from "zod";
import { IndentStatus, LineItemStatus, LeaveType } from "@tusker/db";
import prisma from "@tusker/db";
import { fetchUserPermissions } from "../../../permissions";
import { TasksService } from "../../../server/services/task/tasks.service";
import { LeaveService } from "../../../server/services/leave/leave.service";
import { DailyReportService } from "../../../server/services/daily-report/daily-report.service";
...
const createTask = defineTool({
    ...
    async execute(args, ctx) {
        ...
        const permissions = await fetchUserPermissions(ctx.workspaceId, args.projectId, ctx.userId);
        // In this schema a Task is a parent container: description, assignee,
        // dates and status live on its subtasks, so they are not passed here.
        // The create_subtask tool below carries them in full.
...
async function runTaskUpdate(
    ...
    const permissions = await fetchUserPermissions(ctx.workspaceId, projectId, ctx.userId);
...
const createSubtask = defineTool({
    ...
    async execute(args, ctx) {
        ...
        const permissions = await fetchUserPermissions(ctx.workspaceId, task!.projectId, ctx.userId);
...
async function executeReport(args: z.infer<typeof DailyReportArgs>, ctx: TravisContext): Promise<ToolResult> {
    await validateReportTasks(args, ctx);
    await DailyReportService.submit(ctx.userId, {
        workspaceId: ctx.workspaceId,
...
function executeIndent(status: IndentStatus) {
    return async (args: z.infer<typeof IndentArgs>, ctx: TravisContext): Promise<ToolResult> => {
        ...
        const created = await prisma.$transaction(async (tx) => {
            ...
            if (args.materials?.length) {
                await tx.indentLineItem.createMany({
                ...
                for (const m of args.materials) {
                    await tx.materialCatalog.upsert({
                        ...
                        create: {
                            id: randomUUID(),
                            // This schema gives every catalog entry a human-facing
                            // sequential id, allocated by the shared helper.
                            materialId: await nextMaterialId(tx, ctx.workspaceId),
                            workspaceId: ctx.workspaceId,
                            name: m.materialName,
                            unit: m.unit || "unit",
                            source: "INDENT",
