 ...
 * Handles arrays by sorting them for stable JSON output.
 */
function getFilterHash(filters: Partial<TaskFilters>): string {
  ...
  const hashObj = {
    ...
    assigneeId: normalizeValue(filters.assigneeId),
    tagId: normalizeValue(filters.tagId),
...
async function _getSubTasksByParentIdsInternal(
  ...
  console.log(
    `🔍 [DEBUG] getSubTasksByParentIdsInternal called for parentIds: ${parentTaskIds.join(", ")} in project: ${projectId} view: ${viewMode}`,
  );
  console.log(
    `🔍 [DEBUG] Permissions: isAdmin=${isAdmin}, fullAccessProjectIds=${fullAccessProjectIds.join(",")}, restrictedProjectIds=${restrictedProjectIds.join(",")}`,
  );
  ...
  if (parentTaskIds.length === 1) {
    ...
    console.log(
      `🔍 [DEBUG] singleParentWhere:`,
      JSON.stringify(singleParentWhere, null, 2),
    );
    ...
    console.log(
      `🔍 [DEBUG] Fetched ${rawTasks.length} raw tasks for parent ${parentId}`,
    );

    // Extract parent and project data from the combined query
    const parentData = parentAndProjectData
      ? {
          id: parentAndProjectData.id,
          name: (parentAndProjectData as any).name,
        }
      : null;
    const projectData = (parentAndProjectData as any)?.project || null;

    console.log(
      `🔍 [BATCH_SUBTASKS_EXPAND] parent: ${parentId}, userId: ${userId}`,
    );
    console.log(
      `🔍 [BATCH_SUBTASKS_EXPAND] Project: ${projectData?.name || "Unknown"}`,
    );
    console.log(
      `🔍 [BATCH_SUBTASKS_EXPAND] Found ${rawTasks.length} raw subtasks.`,
    );
    if (rawTasks.length === 0) {
      // Debug: query DB directly to see what statuses exist
      const debugStatuses = await prisma.task.findMany({
        where: { parentTaskId: parentId },
        select: { status: true },
        take: 20,
      });
      console.log(
        `⚠️ [BATCH_SUBTASKS_EXPAND] No subtasks match filter. Parent ${parentId} has subtasks with statuses:`,
        [...new Set(debugStatuses.map((s) => s.status))],
      );
    }

    const hasMore = rawTasks.length > pageSize;
    const finalTasks = (hasMore ? rawTasks.slice(0, pageSize) : rawTasks).map(
      (t) => TasksService.mapToFlatMetadata(t),
    );

    const duration = performance.now() - startTime;

    const nextCursor =
      hasMore && finalTasks.length > 0
        ? {
  ...
  if (duration > 150) {
    console.log(
      `[PERF] Batch Subtasks: ${duration.toFixed(2)}ms for ${parentTaskIds.length} parents`,
    );
  }
  ...
  return parentTaskIds.map((parentTaskId) => {
    ...
    const nextCursor =
      hasMore && pagedSubTasks.length > 0
        ? {
...
const getCachedSubTasksByParentIds = async (
  ...
  const results = await unstable_cache(
    async () =>
      _getSubTasksByParentIdsInternal(
        parentTaskIds,
        workspaceId,
        projectId,
        userId,
  ...
  async (
    ...
    try {
      ...
      if (!skipPermissionsCheck) {
        ...
        } = await TasksService.resolveTaskPermissions(
          workspaceId,
          projectId,
          userId,
        );
      ...
      const results = parentTaskIds.length === 1
        ? await _getSubTasksByParentIdsInternal(
            parentTaskIds,
            workspaceId,
            projectId,
        ...
        : await getCachedSubTasksByParentIds(
            parentTaskIds,
            workspaceId,
            projectId,
            workspaceMemberUserId,
      ...
      const totalSubtasks = results.reduce((sum, r) => sum + r.totalCount, 0);
      console.log(
        `📊 [SUBTASK COUNT] Filter: ${JSON.stringify(filters)} | Total subtasks (all parents): ${totalSubtasks}`
      );
...
export type BatchSubTasksResponse = Awaited<
  ReturnType<typeof getSubTasksByParentIds>
>;
